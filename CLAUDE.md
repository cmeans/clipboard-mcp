...
- **clipboard.py** — Platform-agnostic clipboard abstraction. Auto-detects backend (Wayland `wl-paste`/`wl-copy`, X11 `xclip`, macOS `osascript`/`pbpaste`/`pbcopy`, Windows pywin32 for text formats via `clipboard_win32` + Windows PowerShell for image read/write until Phase 2). All operations are async with 5-second timeout. Exit code 1 means "format not available" (not an error). macOS UTI types and Windows format names are mapped to MIME types in `list_formats`. Supports text read/write and binary image reads.

- **clipboard_win32.py** — Direct Win32 clipboard wrapper via pywin32 (Windows-only dependency, `sys_platform == 'win32'` marker in pyproject.toml). Backs the Windows text/plain, text/html, text/rtf, image/svg+xml, and multi-format paths in `clipboard.py`. Synchronous Win32 API calls (`OpenClipboard` / `EmptyClipboard` / `SetClipboardData` / `GetClipboardData` / `EnumClipboardFormats` / `RegisterClipboardFormat` / `CloseClipboard`) wrapped from `clipboard.py`'s async dispatchers via `asyncio.to_thread`. Replaces the prior PowerShell-subprocess-per-call backend that suffered cross-process read-after-write races and codepage-transcoding fragility. Image read/write paths still on PowerShell pending Phase 2 (DIB ↔ PNG conversion).

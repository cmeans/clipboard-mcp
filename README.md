...
* **Writing multiple MIME types atomically is not supported** on Wayland/X11. `wl-copy` and `xclip` carry a single MIME per invocation, so `clipboard_copy_markdown` writes only `text/html` on those platforms (rich-text targets render; plain-text targets see nothing — call `clipboard_copy(markdown_source)` directly for those). macOS and Windows do support atomic multi-format writes via NSPasteboard / `DataObject`.

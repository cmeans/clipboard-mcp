...
def _get_clipboard_hwnd() -> int:
    """Return a stable HWND to associate with OpenClipboard.
    ...
    owner to NULL. Note that this causes SetClipboardData to fail." Calling
    OpenClipboard() with no argument (or with hwnd=0) yields NULL ownership
    and silently breaks SetClipboardData for *registered custom formats*
    like 'image/svg+xml'. Built-in formats (CF_UNICODETEXT) appear to
    survive because their synthesis path (CF_TEXT / CF_OEMTEXT / CF_LOCALE
    auto-generated by Windows) bootstraps ownership establishment, but the
    custom path silently no-ops.

    Confirmed empirically by the PR #146 verification run
    `mcp-clipboard-windows-e2e-pr146-verify-claude-code-2026-05-09T22:09:11Z`:

      - mc-008 (write_multi text/html + text/plain) PASSED -- the
        text/plain SetClipboardData(CF_UNICODETEXT, ...) bootstraps
        ownership, then the registered HTML Format succeeds.
      - mc-005 / mc-020 (write_text image/svg+xml alone) FAILED with the
        silent-no-op signature -- registered custom format only, no
        ownership bootstrap.
      - mc-103 (direct PowerShell GetDataObject read after the same
        MCP write) SAW the SVG: PowerShell's Set-Clipboard and .NET's
        Clipboard.SetDataObject use the OLE clipboard API
        (OleSetClipboard) internally, which manages ownership
        transparently and doesn't have this issue.

    Fix: associate OpenClipboard with the desktop window. GetDesktopWindow()
    returns a stable HWND that always exists; using it as the clipboard
    owner means EmptyClipboard sets a non-NULL owner and the subsequent
    SetClipboardData calls (for any format, custom or built-in) commit
    correctly. The desktop window has no message loop concerns for our
    use case because we set actual data (not a delayed-render handler)
    and the data persists after our process exits regardless of whether
    the owner window is responsive.
    """
    import win32gui  # type: ignore[import-not-found,import-untyped]

    return int(win32gui.GetDesktopWindow())

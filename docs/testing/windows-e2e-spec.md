...
1. **Identify platform.** Determine whether the agent is running under Claude Code (CC) or Claude Desktop (CD). Record as `platform` in every result entry. Set `learned_from` accordingly: `"claude-code"` or `"claude-desktop"`.
